...
function getLatestPlaywrightVersion() {
  ...
  if (result.status !== 0) {
    throw new Error(
      result.stderr || 'Unable to resolve latest @playwright/test version.'
    );
  }
...
const playwrightTemplateVulnerabilities = getAuditSummary(
  templateDirectories.playwright
);
const cypressTemplateVulnerabilities = getAuditSummary(
  templateDirectories.cypress
);
const vulnerabilities = {
  ...
  low:
    playwrightTemplateVulnerabilities.low + cypressTemplateVulnerabilities.low,
  info:
    playwrightTemplateVulnerabilities.info + cypressTemplateVulnerabilities.info
};
const updateAvailable =
  compareVersions(latestPlaywrightVersion, currentPlaywrightVersion) > 0;
const shouldNotify =
  updateAvailable ||
...
writeOutput(
  'playwright_template_vulnerabilities_total',
  String(playwrightTemplateVulnerabilities.total)
);
writeOutput(
  'playwright_template_vulnerabilities_critical',
  String(playwrightTemplateVulnerabilities.critical)
);
writeOutput(
  'playwright_template_vulnerabilities_high',
  String(playwrightTemplateVulnerabilities.high)
);
writeOutput(
  'playwright_template_vulnerabilities_moderate',
  String(playwrightTemplateVulnerabilities.moderate)
);
writeOutput(
  'playwright_template_vulnerabilities_low',
  String(playwrightTemplateVulnerabilities.low)
);
writeOutput(
  'playwright_template_vulnerabilities_info',
  String(playwrightTemplateVulnerabilities.info)
);
writeOutput(
  'cypress_template_vulnerabilities_total',
  String(cypressTemplateVulnerabilities.total)
);
writeOutput(
  'cypress_template_vulnerabilities_critical',
  String(cypressTemplateVulnerabilities.critical)
);
writeOutput(
  'cypress_template_vulnerabilities_high',
  String(cypressTemplateVulnerabilities.high)
);
writeOutput(
  'cypress_template_vulnerabilities_moderate',
  String(cypressTemplateVulnerabilities.moderate)
);
writeOutput(
  'cypress_template_vulnerabilities_low',
  String(cypressTemplateVulnerabilities.low)
);
writeOutput(
  'cypress_template_vulnerabilities_info',
  String(cypressTemplateVulnerabilities.info)
);
