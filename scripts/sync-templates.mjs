...
const SYNC_MAP = [
  ...
  },
  {
    ...
    targets: [
      ...
      { dest: "templates/cypress-template/config/environments.ts", transforms: [removeApiBaseUrl] }
    ]
  },
  {
    ...
    targets: [
      ...
      { dest: "templates/cypress-template/config/runtime-config.ts", transforms: [removeApiBaseUrl] }
    ]
  },
  // --- data files ---
  {
  ...
  },
  {
    ...
    targets: [
      ...
      { dest: "templates/cypress-template/cypress/support/data/data-factory.ts", transforms: [rewriteDataImports] }
    ]
  }
];
...
function syncCliTemplates() {
  ...
  for (const name of templateNames) {
    ...
    const dest = path.join(ROOT, "tools", "create-qa-patterns", "templates", name);
...
function checkCliTemplateSync(src, dest, name) {
  if (!fs.existsSync(dest)) {
    driftCount += 1;
    console.error(`DRIFT: CLI template missing: tools/create-qa-patterns/templates/${name}`);
    return;
  }

  const srcFiles = collectFiles(src).filter((f) => !CLI_SYNC_EXCLUDE.has(f.split(path.sep)[0]));
  ...
  for (const file of srcFiles) {
    if (!destSet.has(file)) {
      driftCount += 1;
      console.error(`DRIFT: CLI template missing file: tools/create-qa-patterns/templates/${name}/${file}`);
      continue;
    }
    ...
    if (srcContent !== destContent) {
      driftCount += 1;
      console.error(`DRIFT: CLI template differs: tools/create-qa-patterns/templates/${name}/${file}`);
    }
  }
  ...
  for (const file of destFiles) {
    if (!srcSet.has(file)) {
      driftCount += 1;
      console.error(`DRIFT: CLI template has extra file: tools/create-qa-patterns/templates/${name}/${file}`);
    }
  }
}
...
console.log(CHECK_MODE ? "Checking template sync..." : "Syncing templates from qa-patterns-core...");
console.log("");
...
if (CHECK_MODE) {
  console.log("");
  if (driftCount > 0) {
    console.error(`Found ${driftCount} file(s) out of sync. Run: node scripts/sync-templates.mjs`);
    process.exit(1);
  } else {
